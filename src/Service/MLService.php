...
class MLService
{
    private const API_BASE = 'http://127.0.0.1:5000';
    ...
    public function analyzeToxicity(string $text): array
    {
        ...
        try {
            $response = $this->client->request('POST', self::API_BASE . '/predict', [
                'json' => ['text' => $text],
    ...
    public function detectLanguage(string $text): array
    {
        ...
        try {
            $response = $this->client->request('POST', self::API_BASE . '/detect-language', [
                'json' => ['text' => $text],
                'timeout' => self::TIMEOUT,
            ]);
    ...
    public function analyzeSentiment(string $text): array
    {
        ...
        try {
            $response = $this->client->request('POST', self::API_BASE . '/sentiment', [
                'json' => ['text' => $text],
                'timeout' => self::TIMEOUT,
            ]);
            return $response->toArray();
    ...
    public function smartMatch(string $requestText, array $tutors): array
    {
        ...
        try {
            $response = $this->client->request('POST', self::API_BASE . '/smart-match', [
                'json' => [
    ...
    public function classifyRequest(string $title, string $description): array
    {
        ...
        try {
            $response = $this->client->request('POST', self::API_BASE . '/classify', [
                'json' => [
                    'title' => $title,
    ...
    public function suggestReplies(array $recentMessages): array
    {
        ...
        try {
            $response = $this->client->request('POST', self::API_BASE . '/suggest-replies', [
