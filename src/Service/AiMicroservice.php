...
{
    private const API_BASE = 'http://127.0.0.1:8001';
    private const JOURNAL_API_BASE = 'http://127.0.0.1:5006';
    ...
    {
        try {
            $response = $this->client->request('POST', self::API_BASE . '/predict/dropout', [
                'json' => $features,
                'timeout' => 5,
            ]);
    ...
    {
        try {
            $response = $this->client->request('POST', self::API_BASE . '/recommend/courses', [
                'json' => [
                    'user_text' => $userText,
                    'top_n' => $topN
                ],
    ...
    public function analyzeCourse(array $stats): array
    {
        try {
            ...
            $response = $this->client->request('POST', self::API_BASE . '/analyze/course', [
                'json' => $stats,
                'timeout' => 5,
            ]);
            return $response->toArray();
    ...
    {
        try {
            $response = $this->client->request('POST', self::API_BASE . '/chat', [
                'json' => [
                    'message' => $message,
                    'user_id' => $userId
                ],
    ...
    {
        try {
            $response = $this->client->request('POST', self::JOURNAL_API_BASE . '/predict', [
                'json' => ['text' => $text],
                'timeout' => 5,
            ]);
